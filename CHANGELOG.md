...
- **The user-manual cover announced a two-release-old version.**
  `build_pdf.py` held the version as its own hardcoded `APP_VERSION`,
  ...
  The duplicate is now gone rather than merely re-synced: the cover
  version is read from `spody_gui.__version__`, so a release bumps
  three constants instead of four and this particular way of drifting
  no longer exists. It is parsed, not imported &mdash; the script runs
  from a bare interpreter and an arbitrary working directory during
  the release bundle build, before `spody_gui` is importable &mdash;
