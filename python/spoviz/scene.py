...
"""Scene3D: the time-dynamic 3D astrodynamics scene engine.

Pure VTK + numpy -- NO Qt anywhere in this module. Scene3D wires two
layered renderers (Cesium-multi-frustum style) onto a render window
provided by the caller and exposes the spody scene API:

    scene.clear_scene()                          # remove all data props
    scene.add_central_body(radius_km, ...)       # (textured) sphere
    scene.add_trajectory(points_km, color, lw)   # polyline + endpoint markers
    scene.add_animated_trajectory(...)           # + moving marker on a timeline
    scene.set_animation_time(t_s)                # advance every animated prop
    scene.reset_camera()                         # fit-to-scene
    scene.render()                               # repaint

Hosting contract: the caller owns the render window and the
interactor and must bind them together BEFORE constructing Scene3D
(the Qt path gets this for free from QVTKRenderWindowInteractor, see
`spoviz.qt.SceneWidget`; an offscreen path pairs a `vtkRenderWindow`
with OffScreenRenderingOn and a plain `vtkRenderWindowInteractor`).
...
from .textures import ensure_icrf_aligned_skybox, make_image_reader
...
class Scene3D:
    """The spody 3D scene engine, wired onto a caller-provided render
    window. Stateless wrt to which file is currently shown --
    `clear_scene` followed by add_*() calls rebuilds everything for
    the next plot.

    `render_window` and `interactor` must already be bound to each
    other before construction (plain VTK: `interactor.SetRenderWindow
    (render_window)`; Qt: QVTKRenderWindowInteractor does it
    internally). Scene3D only installs its renderers, the trackball
    style and its observers on them -- it never creates or owns a
    window, which is what keeps it embeddable in Qt, offscreen
    batch-render and future hosts alike."""

    def __init__(self, render_window, interactor) -> None:
        ...
        # the FRONT renderer) rather than as a host-toolkit widget on
        # top of the render window. The Qt-on-top approach left
    ...
    def set_overlay_utc_text(self, text: str) -> None:
        ...
        formatting (the scene knows nothing about ET / leap seconds);
        spody's AnalysisPanel converts `et_start + t_anim_s` via
    ...
    def _install_skybox_actor(self) -> None:
        ...
        aligned = ensure_icrf_aligned_skybox(self._skybox_texture)
        reader = make_image_reader(aligned)
        if reader is None:
            return
        reader.Update()
        texture = vtkTexture()
    ...
    def add_central_body(self, radius_km: float,
        ...
        reader = make_image_reader(chosen) if chosen else None
    ...
    def add_animated_trajectory(self, points_km: np.ndarray,
        ...
        texture_reader = (make_image_reader(marker_texture_path)
    ...
    def add_sun_arrow(self, direction: tuple[float, float, float],
        ...
        km (a few central-body radii reads well) so it stays visible
        regardless of the trajectory scale. Shaft/tip dimensions match
        the frame-triad defaults so the Sun arrow reads as one more
        axis in the scene without dominating it."""
    ...
    def add_frame_triad(self, length_km: float,
                        origin_km: tuple[float, float, float] = (0.0, 0.0, 0.0),
                        basis_in_scene=None,
                        colors_xyz: tuple[tuple[float, float, float], ...] = (
                            (1.0, 0.25, 0.25),
    ...
    def add_point(self, position_km,
        ...
        radius (30 km, ~1.7% of the Moon's radius) is visible without
